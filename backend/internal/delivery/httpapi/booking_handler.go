...
// 🔥 ВНИМАНИЕ: GetIDFromContext УДАЛЕНА отсюда — она теперь только в middleware.go
...
func (h *BookingHandler) BookSlot(w http.ResponseWriter, r *http.Request) {
	tgID, err := GetIDFromContext(r.Context()) // Функция из middleware.go
	if err != nil {
		http.Error(w, err.Error(), http.StatusUnauthorized)
		return
	}

	// 🔥 БЕРЕМ РЕАЛЬНОЕ ИМЯ ИЗ TELEGRAM
	tgUser := GetTelegramUserFromContext(r.Context())
	realClientName := strings.TrimSpace(tgUser.FirstName + " " + tgUser.LastName)
	if realClientName == "" {
		realClientName = tgUser.Username
	}
	if realClientName == "" {
		realClientName = "Клиент"
	}
	...
	if err != nil {
		http.Error(w, "bad time format", http.StatusBadRequest)
		return
	}

	err = h.bookingServ.BookSlot(r.Context(), tgID, req.ServiceID, realClientName, req.Price, startTime)
	if err != nil {

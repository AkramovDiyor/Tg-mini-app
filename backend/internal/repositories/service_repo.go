...
func (s *ServiceRepo) CreateService(ctx context.Context, service models.Service) error {
	query := `INSERT INTO services (master_id, name, duration_min, price, is_deleted) VALUES ($1, $2, $3, $4, FALSE)`
...
func (s *ServiceRepo) GetServicesByMasterID(ctx context.Context, masterID int64) ([]models.Service, error) {
	query := `SELECT id, master_id, name, duration_min, price, is_deleted, created_at, updated_at FROM services WHERE master_id = $1 AND is_deleted = FALSE ORDER BY created_at ASC`
	...
	for rows.Next() {
		...
		err := rows.Scan(&svc.ID, &svc.MasterID, &svc.Name, &svc.DurationMin, &svc.Price, &svc.IsDeleted, &svc.CreatedAt, &svc.UpdatedAt)
		if err != nil {
			return nil, err
		}
...
func (s *ServiceRepo) GetServiceByID(ctx context.Context, serviceID int64) (models.Service, error) {
	...
	err := s.db.QueryRow(ctx, query, serviceID).Scan(&service.ID, &service.MasterID, &service.Name, &service.DurationMin, &service.Price)
	if err != nil {
...
func (s *ServiceRepo) DeleteService(ctx context.Context, serviceID int64, masterID int64) error {
	query := `UPDATE services SET is_deleted = TRUE, updated_at = NOW() WHERE id = $1 AND master_id = $2 AND is_deleted = FALSE`

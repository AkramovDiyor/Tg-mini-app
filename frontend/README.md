...
2. Нажми **Menu Button** внизу чата
3. Должна открыться панель мастера

#### Тест клиента:

1. В панели мастера найди ссылку для клиентов
2. Отправь её другу (или себе в "Избранное")
3. Друг кликает → открывается форма записи
4. Записывается → мастер получает уведомление

---

## 🎯 Возможные проблемы

### ❌ ngrok показывает interstitial page

**Решение:** В `frontend/src/services/api.js` добавь заголовок:
```javascript
...
**Решение:** В `backend/internal/delivery/httpapi/router.go` убедись, что CORS разрешает:
```go
...
### ❌ Telegram не передает initData

**Решение:** Открывай приложение **только через Telegram** (Menu Button или Named Web App ссылку), а не напрямую в браузере.

### ❌ Порт 5173 зарезервирован Windows
...
# Использовать другой порт в vite.config.js
server: { port: 3000 }
```
...
PRs приветствуются! Для крупных изменений сначала откройте issue.
